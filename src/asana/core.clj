...
(def ^:dynamic url "https://app.asana.com/api/1.0/")
...
(defn query-map
  ...
  (into {}
        (for [[k v] (concat defaults entries)]
          [(->snake_case_string k) v])))

(defn parse-json
  "Like json/parse-string but doesn't shit itself on nil"
  [s]
...
(defn parse-links
  "Takes the content of the link header from a resp, returns a map of links.
  Not sure if we need this here, I'm using tentacles as a template and some
  of this might be Github-specific."
  [link-body]
  (->> (str/split link-body #",")
       (map parse-link)
       (into {})))
...
(defn update-req
  "Given an httpkit request, and a 'next' url string, merge the next url into the request"
  [req url]
   ...
   (let [query (or query {})
         ...
         exec-request (fn exec-request [req]
                        (let [resp (exec-request-one req)]
                          (if (and all-pages? (-> meta :links :next))
